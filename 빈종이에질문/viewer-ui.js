...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ⭐️ 자동 후킹: "봇 메시지 렌더" 감지기 (MutationObserver)  // 수정사항!!
    // 챗 메시지 컨테이너 후보들 (프로젝트 클래스/ID에 맞춰 추가/수정 가능)      // 수정사항!!
    const CHAT_CONTAINER_SELECTORS = [ '#chat-messages', '.chat-messages', '#messages', '.messages' ]; // 수정사항!!
    const BOT_BUBBLE_SELECTORS     = [ '[data-role="bot"]', '.bot', '.assistant', '.message.bot', '.message.assistant' ]; // 수정사항!!

    function findChatContainer() { // 수정사항!!
    ...
    // 새로 추가되는 "봇 말풍선"을 감지해서 viewer-main.js 쪽으로 CustomEvent 보내기   // 수정사항!!
    function installBotMessageObserver(container) { // 수정사항!!
      const observer = new MutationObserver((mutations) => {
        for (const m of mutations) {
          for (const node of m.addedNodes) {
            if (!(node instanceof HTMLElement)) continue;

            // 후보 셀렉터 중 하나라도 매칭되면 봇 말풍선으로 간주                 // 수정사항!!
            const isBot = BOT_BUBBLE_SELECTORS.some(sel => node.matches?.(sel) || node.querySelector?.(sel)); // 수정사항!!
            if (!isBot) continue;

            // 텍스트 추출(필요 시 프로젝트 구조에 맞춰 커스터마이징)              // 수정사항!!
            let text = "";
            ...
            // 현재 선택된 페르소나 키 추정                                        // 수정사항!!
            const personaKey = document.getElementById('chat-persona-select')?.value || 'professor'; // 수정사항!!

            // viewer-main.js에 설치해둔 리스너로 전달                              // 수정사항!!
            document.dispatchEvent(new CustomEvent('botMessageRendered', { detail: { text, personaKey } })); // 수정사항!!
          }
        }
      });

      observer.observe(container, { childList: true, subtree: true }); // 수정사항!!
      console.log('[hooks] BotMessage observer installed on', container); // 수정사항!!
    } // 수정사항!!

    // 컨테이너가 늦게 그려지는 경우를 대비해 폴링 시도                           // 수정사항!!
    (function waitForChat() { // 수정사항!!
