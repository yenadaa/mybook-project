...
def generateCustomReview(req: https_fn.CallableRequest) -> https_fn.Response:
    """
    하이라이트만 모아 OX/단답/서술형 세트를 생성하는 엔드포인트.
    기존 generateQuiz(MCQ 3개 고정) 대신 이걸 쓰면 full과 동일 스키마로 반환됨.
    """
    global db, openai_client
    if db is None: db = firestore.client()
    ...
    docs = db.collection("highlights")\
             ...
             .where("bookId", "==", book_id).stream()

    texts = []
    for d in docs:
        t = (d.to_dict().get("text") or "").strip()
        # 너무 짧은 하이라이트는 품질이 떨어져서 제외(필요 시 임계값 조정)
        if len(t) >= 20:
            texts.append(t)

    if not texts:
        # 비어 있으면 빈 결과 스키마 반환(프런트 처리 통일)
        return {"summaries": {}, "review": {"ox": [], "short": [], "discussion": []}}

    # 하이라이트를 chunk로 래핑
    chunks = [Chunk(id=f"h{i}", text=tx, section_path=["highlight"]) for i, tx in enumerate(texts)]
    processed_doc = PreprocessedDoc(doc_id=book_id, chunks=chunks)

    # full과 동일 파이프라인/스키마로 생성
    out = generate_base_review(processed_doc, seed=42, model="gpt-4o-mini")
    # pydantic 모델 → dict
