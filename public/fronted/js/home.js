    ...
    function renderHero(userEmail, streakDays, doc) {
        ...
        if (welcomeEl) {
            welcomeEl.innerHTML = `반가워요, <b>${userEmail ? userEmail.split('@')[0] : '게스트'}</b>님!<br>오늘도 지식을 쌓아볼까요?`;
        }

        // 2. 연속 학습일
        const streakEl = document.getElementById("streakDays");
        if (streakEl) {
            streakEl.textContent = String(streakDays || 0);
        }
        ...
        // 문서가 없을 때 처리
        if (!doc) {
            if(document.getElementById("currentDocTitle")) document.getElementById("currentDocTitle").textContent = "선택된 문서가 없습니다";
            if(document.getElementById("currentDocSub")) document.getElementById("currentDocSub").textContent = "아래 목록에서 문서를 선택해주세요.";
            
            const pText = document.getElementById("currentProgressText");
            const pFill = document.getElementById("currentProgressFill");
            const tGoal = document.getElementById("todayGoal");
            
            if(pText) pText.textContent = "0%";
            if(pFill) pFill.style.width = "0%";
            if(tGoal) tGoal.textContent = "-";
            
            renderChips([]);
            
            if (scheduleBtn) {
                scheduleBtn.onclick = () => alert("목록에서 문서를 먼저 선택해주세요.");
            ...
            if(startBtn) {
                ...
                startBtn.textContent = "문서를 선택해주세요";
            }
            return;
        }

        // 4. 문서 정보가 있을 때 렌더링
        if(document.getElementById("currentDocTitle")) document.getElementById("currentDocTitle").textContent = doc.title || "제목 없음";
        if(document.getElementById("currentDocSub")) document.getElementById("currentDocSub").textContent = `마지막 학습: ${formatTs(doc.lastActivityAt)}`;
        ...
        // 오늘의 목표 (🚨 여기가 에러 원인이었습니다. if문 필수!)
        const goalEl = document.getElementById("todayGoal");
        if (goalEl) {
            goalEl.textContent = getNextGoalText(p);
        }
        ...
        if (scheduleBtn) {
            ...
            scheduleBtn.onclick = async () => {
                // calendar.js에 있는 함수를 호출하거나, 직접 import해서 사용
                if (window.Calendar && window.Calendar.addReviewSchedule) {
                    await window.Calendar.addReviewSchedule(doc.docId, doc.title);
                } else {
                    // 만약 window.Calendar가 아직 로드 안 됐다면 직접 import 시도 (fallback)
                    try {
                        const { addReviewSchedule } = await import('./calendar.js');
