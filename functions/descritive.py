...
def pdf_to_preprocessed_doc(
    pdf_bytes: bytes,  # pdf_path 대신 pdf_bytes를 받음
    doc_id: Optional[str] = None,
) -> PreprocessedDoc:  # Dict 대신 Pydantic 모델을 직접 반환
    """
    [챗봇 방식과 통합된 새 파이프라인]
    PDF 바이트를 입력받아, fitz로 텍스트를 추출하고 
    LangChain으로 쪼갠 뒤, PreprocessedDoc 객체로 반환합니다.
    """
# --- 1. 챗봇 방식: fitz로 텍스트 추출 ---
    all_text = ""
    try:
        # ⭐️ [적용] fitz(PyMuPDF)로 바이트 스트림에서 직접 열기
        doc = fitz.open(stream=pdf_bytes, filetype="pdf") 
    except Exception as e:
        print(f"fitz PDF 열기 오류: {e}")
        # 원본 스크립트에 정의된 PdfReadError (Exception 상속) 재사용
        raise PdfReadError(f"fitz PDF 처리 오류: {e}") 

    for page_num in range(len(doc)):
        try:
            page = doc.load_page(page_num)
            text = page.get_text("text") # 챗봇과 동일하게 "text" 사용
            all_text += text
            all_text += "\n\n" # 챗봇과 동일하게 페이지 구분
    ...
    # --- 2. 챗봇 방식: LangChain으로 텍스트 쪼개기 ---
    # ⭐️ [적용] RecursiveCharacterTextSplitter 사용
    try:
        text_splitter = RecursiveCharacterTextSplitter(
            chunk_size=1000,       # 챗봇 코드의 기본값
            chunk_overlap=100,     # 챗봇 코드의 기본값
            separators=["\n\n", "\n", " ", ""] # 챗봇 코드의 기본값
        )
        split_texts: List[str] = text_splitter.split_text(all_text)
    except Exception as e:
        print(f"LangChain 텍스트 분할 중 오류: {e}")
        # 분할 실패 시, 전체 텍스트를 단일 청크로 사용 (안전장치)
        split_texts = [all_text] if all_text else []


    # --- 3. 변환: PreprocessedDoc 객체로 조립 ---
    chunks: List[Chunk] = []
    for i, text_chunk in enumerate(split_texts):
        # ⭐️ [변경] 챗봇 방식은 페이지/섹션 구분이 없음
        new_chunk = Chunk(
            id=f"c{i}",  # 0-based ID
            text=text_chunk,
            # 챗봇 방식(전체 텍스트 기반)은 섹션 구분이 없으므로 None
            section_path=None, 
            anchors=[]
        )
    ...
    # ⭐️ [변경] dict 대신 Pydantic 모델을 직접 반환
