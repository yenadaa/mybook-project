...
from quiz_generator import generate_base_review, PreprocessedDoc, Chunk
import fitz  # PyMuPDF
...
# ✨ [수정] '하이라이트 기반 퀴즈' 함수의 내용을 올바르게 복원합니다.
...
# ✨ [수정] '전체 문서 기반 퀴즈' 함수를 독립적으로 올바르게 정의합니다.
@https_fn.on_call(
    secrets=["OPENAI_API_KEY"],
    cors=options.CorsOptions(cors_origins=["https://mybook-d143d.web.app"])
)
def generateFullDocQuiz(req: https_fn.CallableRequest) -> https_fn.Response:
    """PDF에서 텍스트를 추출하고 퀴즈를 생성합니다. (체크포인트 포함 디버깅 버전)"""
    ...
    print(f"'{book_id}'에 대한 퀴즈 생성 시작 (체크포인트 버전)")
    ...
    try:
        ...
        if not blob.exists():
            raise https_fn.HttpsError(code='not-found', message='[Checkpoint 1 FAILED] Cloud Storage에서 파일을 찾을 수 없습니다.')
        
        pdf_bytes = blob.download_as_bytes()
        print("Checkpoint 1: 파일 다운로드 성공")
    except Exception as e:
        print(f"Checkpoint 1 오류: {e}")
        raise https_fn.HttpsError(code='internal', message=f'[Checkpoint 1 FAILED] 파일 다운로드 중 오류: {str(e)}')

    # --- 체크포인트 2: PyMuPDF로 PDF 열기 및 텍스트 추출 ---
    try:
        ...
        if not chunks:
            raise https_fn.HttpsError(code='not-found', message='[Checkpoint 2 FAILED] PDF에서 텍스트를 추출할 수 없습니다 (이미지 기반 PDF?).')
        
        print("Checkpoint 2: 텍스트 추출 성공")
    except Exception as e:
        print(f"Checkpoint 2 오류: {e}")
        raise https_fn.HttpsError(code='internal', message=f'[Checkpoint 2 FAILED] PDF 처리(PyMuPDF) 중 오류: {str(e)}')
    ...
    try:
        ...
        raise https_fn.HttpsError(code='internal', message=f'[Checkpoint 3 FAILED] AI 퀴즈 생성 중 오류: {str(e)}')
...
def runOcrOnSelection(req: https_fn.CallableRequest) -> https_fn.Response:
    ...
    try:
        ...
        #의 전체 내용을 확실히 기록하기 위해 traceback을 사용합니다.
        print("--- ERROR TRACEBACK ---")
        traceback.print_exc()
        print("--- END TRACEBACK ---")
        
        error_message = f"서버 오류가 발생했습니다. 로그를 확인해주세요. 원인: {str(e)}"
