...
function updateOcrInteraction(){
  // OCR 레이어
  document.querySelectorAll('.ocr-layer').forEach(layer => {
    layer.style.pointerEvents = (selectMode === 'pen') ? 'auto' : 'none';
  });

  // PDF.js 텍스트 레이어
  document.querySelectorAll('.textLayer').forEach(layer => {
    layer.style.pointerEvents = (selectMode === 'pen') ? 'auto' : 'none';
    // 시각적으로는 이미 CSS에서 투명 처리되어 있음
  });
}
